import '../play_option4_screen/widgets/playoptionlist4_item_widget.dart';
import 'bloc/play_option4_bloc.dart';
import 'models/play_option4_model.dart';
...
class PlayOption4Screen extends StatelessWidget {
  const PlayOption4Screen({Key? key}) : super(key: key);

  static Widget builder(BuildContext context) {
    return BlocProvider<PlayOption4Bloc>(
        create: (context) => PlayOption4Bloc(
            PlayOption4State(playOption4ModelObj: PlayOption4Model()))
          ..add(PlayOption4InitialEvent()),
        child: PlayOption4Screen());
  }
  ...
  Widget _buildPlayOptionList(BuildContext context) {
    return SizedBox(
        height: 174.v,
        child:
            BlocSelector<PlayOption4Bloc, PlayOption4State, PlayOption4Model?>(
                selector: (state) => state.playOption4ModelObj,
                builder: (context, playOption4ModelObj) {
                  return ListView.separated(
                      ...
                      itemCount:
                          playOption4ModelObj?.playoptionlist4ItemList.length ??
                              0,
                      itemBuilder: (context, index) {
                        Playoptionlist4ItemModel model = playOption4ModelObj
                                ?.playoptionlist4ItemList[index] ??
                            Playoptionlist4ItemModel();
                        return Playoptionlist4ItemWidget(model);
                      });
                }));
  }
  ...
  /// Navigates to the sleepMusic4Screen when the action is triggered.
  onTapPlayButton(BuildContext context) {
    NavigatorService.pushNamed(
      AppRoutes.sleepMusic4Screen,
    );
  }
}
