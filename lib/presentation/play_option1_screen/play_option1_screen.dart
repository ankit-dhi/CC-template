import '../play_option1_screen/widgets/playoptionlist1_item_widget.dart';
import 'bloc/play_option1_bloc.dart';
import 'models/play_option1_model.dart';
...
class PlayOption1Screen extends StatelessWidget {
  const PlayOption1Screen({Key? key}) : super(key: key);

  static Widget builder(BuildContext context) {
    return BlocProvider<PlayOption1Bloc>(
        create: (context) => PlayOption1Bloc(
            PlayOption1State(playOption1ModelObj: PlayOption1Model()))
          ..add(PlayOption1InitialEvent()),
        child: PlayOption1Screen());
  }
  ...
  Widget _buildPlayOptionList(BuildContext context) {
    return SizedBox(
        height: 174.v,
        child:
            BlocSelector<PlayOption1Bloc, PlayOption1State, PlayOption1Model?>(
                selector: (state) => state.playOption1ModelObj,
                builder: (context, playOption1ModelObj) {
                  return ListView.separated(
                      ...
                      itemCount:
                          playOption1ModelObj?.playoptionlist1ItemList.length ??
                              0,
                      itemBuilder: (context, index) {
                        Playoptionlist1ItemModel model = playOption1ModelObj
                                ?.playoptionlist1ItemList[index] ??
                            Playoptionlist1ItemModel();
                        return Playoptionlist1ItemWidget(model);
                      });
                }));
  }
  ...
  /// Navigates to the sleepMusic1Screen when the action is triggered.
  onTapPlayButton(BuildContext context) {
    NavigatorService.pushNamed(
      AppRoutes.sleepMusic1Screen,
    );
  }
}
