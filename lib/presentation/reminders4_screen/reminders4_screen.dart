import '../reminders4_screen/widgets/chipviewsection8_item_widget.dart';
import 'bloc/reminders4_bloc.dart';
import 'models/chipviewsection8_item_model.dart';
import 'models/reminders4_model.dart';
import 'package:ankit_s_application110/core/app_export.dart';
import 'package:ankit_s_application110/widgets/custom_elevated_button.dart';
import 'package:flutter/material.dart';

class Reminders4Screen extends StatelessWidget {
  const Reminders4Screen({Key? key}) : super(key: key);

  static Widget builder(BuildContext context) {
    return BlocProvider<Reminders4Bloc>(
        create: (context) => Reminders4Bloc(
            Reminders4State(reminders4ModelObj: Reminders4Model()))
          ..add(Reminders4InitialEvent()),
        child: Reminders4Screen());
  }
  ...
  Widget _buildChipViewSection(BuildContext context) {
    return Align(
        alignment: Alignment.center,
        child: BlocSelector<Reminders4Bloc, Reminders4State, Reminders4Model?>(
            selector: (state) => state.reminders4ModelObj,
            builder: (context, reminders4ModelObj) {
              return Wrap(
                  runSpacing: 14.75.v,
                  spacing: 14.75.h,
                  children: List<Widget>.generate(
                      reminders4ModelObj?.chipviewsection8ItemList.length ?? 0,
                      (index) {
                    Chipviewsection8ItemModel model =
                        reminders4ModelObj?.chipviewsection8ItemList[index] ??
                            Chipviewsection8ItemModel();
                    return Chipviewsection8ItemWidget(model,
                        onSelectedChipView: (value) {
                      context.read<Reminders4Bloc>().add(
  ...
  /// Navigates to the home4Screen when the action is triggered.
  onTapSaveButtonSection(BuildContext context) {
    NavigatorService.pushNamed(
      AppRoutes.home4Screen,
    );
  }
}
