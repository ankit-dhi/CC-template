import '../reminders3_screen/widgets/chipviewsection6_item_widget.dart';
import 'bloc/reminders3_bloc.dart';
import 'models/chipviewsection6_item_model.dart';
import 'models/reminders3_model.dart';
import 'package:ankit_s_application110/core/app_export.dart';
import 'package:ankit_s_application110/widgets/custom_elevated_button.dart';
import 'package:flutter/material.dart';

class Reminders3Screen extends StatelessWidget {
  const Reminders3Screen({Key? key}) : super(key: key);

  static Widget builder(BuildContext context) {
    return BlocProvider<Reminders3Bloc>(
        create: (context) => Reminders3Bloc(
            Reminders3State(reminders3ModelObj: Reminders3Model()))
          ..add(Reminders3InitialEvent()),
        child: Reminders3Screen());
  }
  ...
  Widget _buildChipViewSection(BuildContext context) {
    return Align(
        alignment: Alignment.center,
        child: BlocSelector<Reminders3Bloc, Reminders3State, Reminders3Model?>(
            selector: (state) => state.reminders3ModelObj,
            builder: (context, reminders3ModelObj) {
              return Wrap(
                  runSpacing: 14.75.v,
                  spacing: 14.75.h,
                  children: List<Widget>.generate(
                      reminders3ModelObj?.chipviewsection6ItemList.length ?? 0,
                      (index) {
                    Chipviewsection6ItemModel model =
                        reminders3ModelObj?.chipviewsection6ItemList[index] ??
                            Chipviewsection6ItemModel();
                    return Chipviewsection6ItemWidget(model,
                        onSelectedChipView: (value) {
                      context.read<Reminders3Bloc>().add(
  ...
  /// Navigates to the home3Screen when the action is triggered.
  onTapSaveButtonSection(BuildContext context) {
    NavigatorService.pushNamed(
      AppRoutes.home3Screen,
    );
  }
}
