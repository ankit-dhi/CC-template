import '../play_option_screen/widgets/playoptionlist_item_widget.dart';
import 'bloc/play_option_bloc.dart';
import 'models/play_option_model.dart';
...
class PlayOptionScreen extends StatelessWidget {
  ...
  static Widget builder(BuildContext context) {
    return BlocProvider<PlayOptionBloc>(
        create: (context) => PlayOptionBloc(
            PlayOptionState(playOptionModelObj: PlayOptionModel()))
          ..add(PlayOptionInitialEvent()),
        child: PlayOptionScreen());
  }
  ...
  Widget _buildPlayOptionList(BuildContext context) {
    return SizedBox(
        height: 174.v,
        child: BlocSelector<PlayOptionBloc, PlayOptionState, PlayOptionModel?>(
            selector: (state) => state.playOptionModelObj,
            builder: (context, playOptionModelObj) {
              return ListView.separated(
                  ...
                  itemCount:
                      playOptionModelObj?.playoptionlistItemList.length ?? 0,
                  itemBuilder: (context, index) {
                    PlayoptionlistItemModel model =
                        playOptionModelObj?.playoptionlistItemList[index] ??
                            PlayoptionlistItemModel();
                    return PlayoptionlistItemWidget(model);
                  });
            }));
  }
  ...
  /// Navigates to the sleepMusicScreen when the action is triggered.
  onTapPlayButton(BuildContext context) {
    NavigatorService.pushNamed(
      AppRoutes.sleepMusicScreen,
    );
  }
}
