import '../course_details1_screen/widgets/coursedetails1_item_widget.dart';
import 'bloc/course_details1_bloc.dart';
import 'models/course_details1_model.dart';
...
class CourseDetails1Screen extends StatelessWidget {
  const CourseDetails1Screen({Key? key}) : super(key: key);

  static Widget builder(BuildContext context) {
    return BlocProvider<CourseDetails1Bloc>(
        create: (context) => CourseDetails1Bloc(
            CourseDetails1State(courseDetails1ModelObj: CourseDetails1Model()))
          ..add(CourseDetails1InitialEvent()),
        child: CourseDetails1Screen());
  }
  ...
  Widget _buildCourseDetails(BuildContext context) {
    return Align(
        ...
        child: Padding(
            ...
            child: BlocSelector<CourseDetails1Bloc, CourseDetails1State,
                    CourseDetails1Model?>(
                selector: (state) => state.courseDetails1ModelObj,
                builder: (context, courseDetails1ModelObj) {
                  return ListView.separated(
                      ...
                      itemCount: courseDetails1ModelObj
                              ?.coursedetails1ItemList.length ??
                          0,
                      itemBuilder: (context, index) {
                        Coursedetails1ItemModel model = courseDetails1ModelObj
                                ?.coursedetails1ItemList[index] ??
                            Coursedetails1ItemModel();
                        return Coursedetails1ItemWidget(model, onTapVector: () {
                          onTapVector(context);
                        });
                      });
                })));
  ...
  /// Navigates to the meditateV3Screen when the action is triggered.
  onTapVector(BuildContext context) {
    NavigatorService.pushNamed(AppRoutes.meditateV3Screen);
  }
