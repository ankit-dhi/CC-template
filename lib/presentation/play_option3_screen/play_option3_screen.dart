import '../play_option3_screen/widgets/playoptionlist3_item_widget.dart';
import 'bloc/play_option3_bloc.dart';
import 'models/play_option3_model.dart';
...
class PlayOption3Screen extends StatelessWidget {
  const PlayOption3Screen({Key? key}) : super(key: key);

  static Widget builder(BuildContext context) {
    return BlocProvider<PlayOption3Bloc>(
        create: (context) => PlayOption3Bloc(
            PlayOption3State(playOption3ModelObj: PlayOption3Model()))
          ..add(PlayOption3InitialEvent()),
        child: PlayOption3Screen());
  }
  ...
  Widget _buildPlayOptionList(BuildContext context) {
    return SizedBox(
        height: 174.v,
        child:
            BlocSelector<PlayOption3Bloc, PlayOption3State, PlayOption3Model?>(
                selector: (state) => state.playOption3ModelObj,
                builder: (context, playOption3ModelObj) {
                  return ListView.separated(
                      ...
                      itemCount:
                          playOption3ModelObj?.playoptionlist3ItemList.length ??
                              0,
                      itemBuilder: (context, index) {
                        Playoptionlist3ItemModel model = playOption3ModelObj
                                ?.playoptionlist3ItemList[index] ??
                            Playoptionlist3ItemModel();
                        return Playoptionlist3ItemWidget(model);
                      });
                }));
  }
  ...
  /// Navigates to the sleepMusic3Screen when the action is triggered.
  onTapPlayButton(BuildContext context) {
    NavigatorService.pushNamed(
      AppRoutes.sleepMusic3Screen,
    );
  }
}
