import '../course_details3_screen/widgets/coursedetailslist1_item_widget.dart';
import 'bloc/course_details3_bloc.dart';
import 'models/course_details3_model.dart';
...
class CourseDetails3Screen extends StatelessWidget {
  const CourseDetails3Screen({Key? key}) : super(key: key);

  static Widget builder(BuildContext context) {
    return BlocProvider<CourseDetails3Bloc>(
        create: (context) => CourseDetails3Bloc(
            CourseDetails3State(courseDetails3ModelObj: CourseDetails3Model()))
          ..add(CourseDetails3InitialEvent()),
        child: CourseDetails3Screen());
  }
  ...
  Widget _buildCourseDetailsList(BuildContext context) {
    return Align(
        ...
        child: Padding(
            ...
            child: BlocSelector<CourseDetails3Bloc, CourseDetails3State,
                    CourseDetails3Model?>(
                selector: (state) => state.courseDetails3ModelObj,
                builder: (context, courseDetails3ModelObj) {
                  return ListView.separated(
                      ...
                      itemCount: courseDetails3ModelObj
                              ?.coursedetailslist1ItemList.length ??
                          0,
                      itemBuilder: (context, index) {
                        Coursedetailslist1ItemModel model =
                            courseDetails3ModelObj
                                    ?.coursedetailslist1ItemList[index] ??
                                Coursedetailslist1ItemModel();
                        return Coursedetailslist1ItemWidget(model,
                            onTapVector: () {
                          onTapVector(context);
                        });
                      });
  ...
  /// Navigates to the meditateV5Screen when the action is triggered.
  onTapVector(BuildContext context) {
    NavigatorService.pushNamed(AppRoutes.meditateV5Screen);
  }
