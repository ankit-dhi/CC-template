import '../reminders2_screen/widgets/chipviewsection4_item_widget.dart';
import 'bloc/reminders2_bloc.dart';
import 'models/chipviewsection4_item_model.dart';
import 'models/reminders2_model.dart';
import 'package:ankit_s_application110/core/app_export.dart';
import 'package:ankit_s_application110/widgets/custom_elevated_button.dart';
import 'package:flutter/material.dart';

class Reminders2Screen extends StatelessWidget {
  const Reminders2Screen({Key? key}) : super(key: key);

  static Widget builder(BuildContext context) {
    return BlocProvider<Reminders2Bloc>(
        create: (context) => Reminders2Bloc(
            Reminders2State(reminders2ModelObj: Reminders2Model()))
          ..add(Reminders2InitialEvent()),
        child: Reminders2Screen());
  }
  ...
  Widget _buildChipViewSection(BuildContext context) {
    return Align(
        alignment: Alignment.center,
        child: BlocSelector<Reminders2Bloc, Reminders2State, Reminders2Model?>(
            selector: (state) => state.reminders2ModelObj,
            builder: (context, reminders2ModelObj) {
              return Wrap(
                  runSpacing: 14.75.v,
                  spacing: 14.75.h,
                  children: List<Widget>.generate(
                      reminders2ModelObj?.chipviewsection4ItemList.length ?? 0,
                      (index) {
                    Chipviewsection4ItemModel model =
                        reminders2ModelObj?.chipviewsection4ItemList[index] ??
                            Chipviewsection4ItemModel();
                    return Chipviewsection4ItemWidget(model,
                        onSelectedChipView: (value) {
                      context.read<Reminders2Bloc>().add(
  ...
  /// Navigates to the home2Screen when the action is triggered.
  onTapSaveButtonSection(BuildContext context) {
    NavigatorService.pushNamed(
      AppRoutes.home2Screen,
    );
  }
}
